...
def print_inference_result(
    ...
    corrected_delta_deg: np.ndarray,
    ...
    print("  关节名称            │         当前状态 │          模型增量 │          校正后增量 │          目标角度")
    print("  " + "─" * 23 + "┼" + "─" * 14 + "┼" + "─" * 14 + "┼" + "─" * 14 + "┼" + "─" * 14)

    for i, name in enumerate(JOINT_NAMES_8D):
        def fmt(val):
            return f"{val:+.3f}°"
        print(f"  {name:<21} │ {fmt(state_deg[i]):>10} │ {fmt(model_delta_deg[i]):>10} │ {fmt(corrected_delta_deg[i]):>10} │ {fmt(target_action_deg[i]):>10}")
...
class RealtimeInferenceClient:
    ...
    def run_inference(self):
        ...
        # ========== 解析输出 ==========
        model_delta = result["actions"][0]  # shape: (50, 8), 取第一帧
        ...
        # ========== 打印推理结果 ==========
        print(f"\n{'='*70}")
        print(f"[推理结果 #{infer_num}]")
        print(f"{'='*70}")
        print(f"校正参数 (deg): {[f'{x:.2f}' for x in FIXED_CORRECTION_DEG.tolist()]}")

        print(f"\n  {'关节':<22} │ {'当前(°)':<10} │ {'模型增量':<10} │ {'校正增量':<10} │ {'目标角度':<10}")
        print(f"  {'-'*22}─┼{'─'*11}┼{'─'*11}┼{'─'*11}┼{'─'*11}")

        for i, name in enumerate(JOINT_NAMES_8D):
            print(f"  {name:<22} │ {state_deg[i]:>+9.3f} │ {model_delta_deg[i]:>+10.3f} │ {corrected_delta_deg[i]:>+10.3f} │ {target_action_deg[i]:>+9.3f}")
        ...
        # ========== 打印即将发送的动作 ==========
        print(f"\n{'='*70}")
        print(f"[发送动作 #{infer_num}]")
        print(f"{'='*70}")
        print(f"  obs_seq: {action_msg['obs_seq']}")
        print(f"  joints_right: {[f'{x:.3f}' for x in action_msg['joints_right']]}")
        print(f"  dexhand_right: {action_msg['dexhand_right']} ({'闭合' if action_msg['dexhand_right'] >= 0.5 else '张开'})")
    ...
    def run(self, listen_port: int):
        ...
        print(f"使用校正: {self.use_correction}")
        print("=" * 70)
        ...
        print("操作说明:")
        print("  按 Enter/回车: 使用最新观测执行推理并发送给机器人")
        print("  输入 q: 退出程序")
        print("=" * 70)
        ...
        try:
            while self._running:
                ...
                try:
                    user_input = input("按回车执行推理，或输入 q 退出: ").strip()
                except EOFError:
                    break
