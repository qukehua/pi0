"""将智平方单臂机器人 HDF5 数据转换为 LeRobot (OpenPI) 数据集格式的一体化脚本。

功能特点：
1、将HDF5格式的文件，转为lerobot的2.1版本的格式。
2、只保留2路摄像头：头部和右手腕。
3、单臂7自由度 + 灵巧手1自由度 = 8维状态。
4、只保留右臂关节动作和右手机动手。

注意：此脚本专门用于只使用右臂的场景，不需要左臂数据。
"""
...
# 2、路径修改
INPUT_DIR = r'/share/0xyj/datasate_baocun/task_602_14双臂+4路摄像头+2灵巧手+竖直箱子提放/ds_571_31_602_14双臂+4路摄像头+2灵巧手+竖直箱子提放'          # 输入 HDF5 文件所在目录
OUTPUT_DIR = r'/share/0xyj/model3_openpi0.5/hdf5_to_lerobot_data/lerobot_dataset'      # 输出 LeRobot 数据集的根目录
FPS = 30
CHUNK_SIZE = 1000

# 3、【重中之重】：您的 VLA (Pi0.5、pi0) 语言指令集合
TASK_INSTRUCTIONS = [
    "use the right arm to move the basket to another region"
]

# 4、一键启动转换脚本       python /share/0xyj/model3_openpi0.5/hdf5_to_lerobot_data/convert_right_arm_only.py
...
def process_dataset():
    ...
    print("已开启耗时统计：将显示每集耗时、总耗时与预计剩余时间 ETA")
    ...
    for episode_idx, file_path in enumerate(files):
        ...
        print(f"正在处理第 {episode_idx} 个剧集: {file_path}")

        with h5py.File(file_path, 'r') as f:
            # -------------------------------------------------------------
            # A. 读取本体传感器数据 - 只保留右臂数据（去掉左臂）
            # -------------------------------------------------------------
    ...
    info = {
        ...
        "features": {
            "observation.state": {"dtype": "float32", "shape": [8], "names": {"motors": motor_names}, "fps": FPS},
            "observation.effort": {"dtype": "float32", "shape": [8], "names": {"motors": motor_names}, "fps": FPS},
            "action": {"dtype": "float32", "shape": [8], "names": {"motors": motor_names}, "fps": FPS},
