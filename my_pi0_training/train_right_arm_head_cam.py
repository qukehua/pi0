#!/usr/bin/env python3
"""Pi0 微调训练脚本 -- 右臂 8 维 + 头部相机

使用方法:
    cd /share/0xyj/model3_openpi0.5/openpi-main
    CUDA_VISIBLE_DEVICES=0 /share/0xyj/model3_openpi0.5/openpi-main/.venv/bin/python3.11 \
        /share/0xyj/model3_openpi0.5/my_pi0_training/train_right_arm_head_cam.py \
        pi0_right_arm_head_cam --exp_name=head_cam_v1

目录结构:
    /share/0xyj/model3_openpi0.5/
    ├── openpi-main/            # 官方仓库（不修改）
    ├── my_pi0_training/        # 本训练模块（当前目录）
    │   ├── train_right_arm_head_cam.py  # 本文件
    │   ├── my_config_right_arm_head_cam.py  # 训练配置
    │   └── compute_norm_stats.py
    ├── lerobot_dataset/        # 数据集（129条）
    └── pi0_base/               # 预训练权重
"""
...
def apply_patches():
    ...
    def patched_load_metadata(self):
        self.info = ds_utils.load_info(self.root)
        # 跳过版本兼容性检查（避免网络请求）
        # load_tasks 依赖 tasks.jsonl 文件，如果不存在则创建默认任务
        try:
            self.tasks, self.task_to_task_index = ds_utils.load_tasks(self.root)
        except (FileNotFoundError, Exception) as e:
            _log_once(f"tasks.jsonl 不存在，创建默认任务: {e}")
            self.tasks = [{"task_name": "default_task"}]
            self.task_to_task_index = {"default_task": 0}

        # load_episodes 可能依赖 episodes.jsonl 或 episodes/chunk-xxx/file-xxx.parquet (v3.0格式)
        try:
            self.episodes = ds_utils.load_episodes(self.root)
            # v3.0 可能返回列表，需要转换为字典格式 {ep_index: {"length": N, ...}}
            if isinstance(self.episodes, list):
                # 从 parquet 文件读取每个 episode 的长度
                from pathlib import Path
                import pandas as pd
                chunks_size = self.info.get('chunks_size', 1000)
                episode_dicts = {}
                for ep_idx in self.episodes:
                    ...
                    parquet_path = self.root / "data" / f"chunk-{ep_chunk:03d}" / f"file-{file_index:03d}.parquet"
                    if parquet_path.exists():
                        df = pd.read_parquet(parquet_path)
                        episode_dicts[ep_idx] = {"length": len(df)}
                    else:
                        episode_dicts[ep_idx] = {"length": 0}
                self.episodes = episode_dicts
        except (FileNotFoundError, Exception) as e:
            _log_once(f"episodes.jsonl 不存在，使用 info.json 中的 total_episodes: {e}")
            # v3.0 格式：episodes 存储在 meta/episodes/chunk-xxx/file-xxx.parquet
            # 从 info.json 获取 total_episodes
            import pandas as pd
            from pathlib import Path
            chunks_size = self.info.get('chunks_size', 1000)
            total_eps = self.info.get('total_episodes', 1)
            episode_dicts = {}
...
def init_wandb(config: _config.TrainConfig, resuming: bool):
    ...
    if resuming and wandb_id_file.exists():
        ...
        wandb.init(
            id=run_id,
            resume="must",
            project="pi0_right_arm_head_cam",
            entity=None,
        )
    else:
        wandb.init(
...
def main(config: _config.TrainConfig):
    init_logging()
    logging.info(f"===== Pi0 右臂 8 维 + 头部相机训练启动 =====")
